...
type AnalysisTask struct {
	...
	Keywords    []string           `bson:"keywords" qqmsg:"关键词"`     // an array about search keywords, Keywords.0 is used to send request to mercari
	Interval    int                `bson:"interval" qqmsg:"搜索间隔"`    // seconds between task run repeatedly
	TargetPrice [2]int             `bson:"targetPrice" qqmsg:"目标价格"` // Optional, items that prices lower than this will be selected into result. default(0 or negative number) means any price is accepted.
	MaxPage     int                `bson:"maxPage" qqmsg:"搜索页数"`     // I perfer 30 items in one page(same as mercari default), so only page count is provided to control total items.
