...
type AnalysisTask struct {
	...
	MustMatch   []string           `bson:"mustMatch"    json:"mustMatch"`   // keywords that must match
...
func (t *AnalysisTask) FormatSimplifiedChinese() string {
	return fmt.Sprintf("任务ID:%v\n号主QQ:%v\n关键词:%s\n目标价格:%v~%v\n最大页数:%v 搜索间隔:%v",
		t.TaskID, t.Owner, tools.ConcatKeyword(t.Keywords), t.TargetPrice[0], t.TargetPrice[1], t.MaxPage, t.Interval)
}
